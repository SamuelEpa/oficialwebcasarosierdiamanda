...
export async function PUT(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  if (!(await requireAdminApi())) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  try { const item = await updateTeacher((await ctx.params).id, await request.json()); if (!item) return NextResponse.json({ error: "No encontrado" }, { status: 404 }); refreshTeacherViews(); return NextResponse.json({ teacher: item }); }
...
export async function PATCH(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (body.action === "duplicate") {
    const item = await duplicateTeacher(id);
    if (!item) return NextResponse.json({ error: "No encontrado" }, { status: 404 });
    refreshTeacherViews();
    return NextResponse.json({ teacher: item });
  }
  if (body.action === "trash") {
    const item = await moveTeacherToTrash(id);
    if (!item) return NextResponse.json({ error: "No encontrado" }, { status: 404 });
    refreshTeacherViews();
    return NextResponse.json({ teacher: item });
  }
  if (body.action === "restore") {
    const item = await restoreTeacher(id);
    if (!item) return NextResponse.json({ error: "No encontrado" }, { status: 404 });
    refreshTeacherViews();
    return NextResponse.json({ teacher: item });
  }
  ...
  const nextStatus =
    body.action === "publish" ? "published" : body.action === "archive" ? "archived" : body.action === "draft" ? "draft" : null;
  if (!nextStatus) return NextResponse.json({ error: "Acción no válida" }, { status: 400 });

  const updated = await updateTeacher(id, { ...item, status: nextStatus });
  refreshTeacherViews();
  return NextResponse.json({ teacher: updated });
}
export async function DELETE(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  if (!(await requireAdminApi())) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const ok = await deleteTeacherPermanently((await ctx.params).id); if (!ok) return NextResponse.json({ error: "No encontrado" }, { status: 404 }); refreshTeacherViews(); return NextResponse.json({ ok: true });
}
