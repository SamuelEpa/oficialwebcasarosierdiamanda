    ...
    <section className="class-detail__facts class-detail__facts--fit mt-7.5 grid grid-cols-2 gap-[24px] items-start h-fit max-[992px]:grid-cols-1 max-[640px]:grid-cols-1" aria-label="Precio y duracion">
      {hasPrices ? (
        <div className="class-detail__fact-block p-[22px_24px] bg-[#fffdf9] rounded-2xl border border-[rgba(147,124,97,0.14)] max-[992px]:p-[18px_19px]">
          <h2 className="m-0 mb-3 text-[#3d3935] text-[26px]/[1.05] [font-family:var(--font-display)] uppercase tracking-[0.03em] max-[992px]:text-[21px]">Precio</h2>
      ...
      {hasDuration ? (
        <div className="class-detail__fact-block p-[22px_24px] bg-[#fffdf9] rounded-2xl border border-[rgba(147,124,97,0.14)] max-[992px]:p-[18px_19px]">
          <h2 className="m-0 mb-3 text-[#3d3935] text-[26px]/[1.05] [font-family:var(--font-display)] uppercase tracking-[0.03em] max-[992px]:text-[21px]">Duracion</h2>
          {item.duration?.trim() ? <p className="class-detail__duration m-0 mb-3.5 text-[#b87352] text-[clamp(14px,1.45vw,16px)] font-normal leading-[1.45]">{item.duration}</p> : null}
              ...
              {item.schedule.map((schedule, scheduleIndex) => (
                <div className="class-detail__schedule-item" key={`${schedule.day}-${scheduleIndex}`}>
                  ...
                  {schedule.slots.map((slot, slotIndex) => (
                    <p className="class-detail__schedule-item m-0 mb-[4px] text-[#68615a] text-[clamp(13px,1.35vw,15px)] leading-normal font-light [font-family:var(--font-menu)] max-[992px]:text-[13px]" key={`${slot}-${slotIndex}`}>{slot}</p>
                  ))}
                </div>
              ))}
            </div>
