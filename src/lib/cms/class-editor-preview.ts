...
import type { FooterComponent, Form, SocialGallery } from "@/lib/cms/types";
...
export interface ClassEditorPreviewChrome {
  ...
  footerContactForm: Form | null;
  siteContact: SiteSettings["contact"];
}

export async function getClassEditorPreviewChrome(): Promise<ClassEditorPreviewChrome> {
  const [navigationItems, settings, socialGallery, footer, footerContactForm] = await Promise.all([
